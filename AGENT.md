...
Aturan mengikat untuk agent yang bekerja di repo ini (project-manager, technical-lead-system-analyst, technical-writer, dan role lain yang menulis artifact governance). Role-spesifik: baca `.claude/agents/<role>.md`. Detail arsitektur: `docs/architecture/M2S-VSH-Lite-v0.1.0-Architecture.md`.
...
- 23 nilai taskStatus (§33 / ADR-011). Tiap status punya **satu penulis** — tabel owner ADR-011.
...
- Merge ke `main` dan keputusan irreversible = **manusia** (ADR-001). Agent tidak merge PR.
