...
Stack: **Go 1.26** (module `github.com/Mind2Screen-Dev-Team/m2s-vsh-platform`), deps `santhosh-tekuri/jsonschema/v6` + `gopkg.in/yaml.v3`. CLI runner `m2s` di `cmd/m2s/`, dipanggil via `scripts/<subcommand>.sh`, bukan `bin/m2s` langsung.
...
Runner dibangun lokal (`make build`) — `bin/` gitignored (ADR-004).
...
| `docs/` | `adr/` (ADR-001..012), `architecture/` (master spec 2600+ baris), `decisions/`, `operator/` (runbook), `system-analysis/` |
| `scripts/` | 12 thin wrapper `m2s <subcommand>`, `_runner.sh` preamble, `pipeline.sh` (orchestrator), `project-kickoff.sh`, `review.sh`, `qa.sh`, `gh-app-token.sh` |
| `templates/` | Kanonik: `agents/`, `artifacts/`, `github/` (workflow, rulesets, CODEOWNERS), `governance/`, `rules/`, `skills/` |
...
5. **Merge manusia** — PR target `develop`, merge ke `main` **selalu manusia** (ADR-001).

Status machine: 23 nilai taskStatus (§33, ADR-011). Setiap status punya **tepat satu penulis** — tabel owner di ADR-011:
...
- Dokumentasi detail: `docs/architecture/M2S-VSH-Lite-v0.1.0-Architecture.md` (master spec), ADR, `docs/operator/*` (runbook), README-PROGRESS.md (roadmap Phase 0-8).
