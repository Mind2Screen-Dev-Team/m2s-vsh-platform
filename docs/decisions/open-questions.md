...
| V-10 | Apakah `bypass_mode: pull_request` pada ruleset `update` mengizinkan GitHub App **merge** PR | 🔴 **TIDAK** (diuji 2 Agustus 2026, backend). Approver App `m2s-approver` (id 4461262, bypass `Integration` `pull_request`) dapat **submit review APPROVE** (`m2s-approver[bot]` → `{state: APPROVED}`) tetapi **merge** PR yang sudah di-review tetap ditolak: `405 Repository rule violations, Cannot update this protected ref`. `bypass_mode: pull_request` hanya mencakup update ref via PR (push), **bukan** `PUT /pulls/{n}/merge` (update ref base). Implikasi: model ADR-001 #5 mengandaikan approver dapat merge — butuh konfigurasi tambahan (ruleset izinkan merge + `required_approving_review_count ≥ 1`), bukan `pull_request` saja. **Menunggu uji mitigasi** (backend, lihat org-migration.md) |
